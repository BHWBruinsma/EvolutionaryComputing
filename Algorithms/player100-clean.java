...
public class player100 implements ContestSubmission
{
    ...
    static int parentsSize = 50; //needs to be even number and >= than 2*populationSize when (lambda, mu) selection is used
	static double s = 1.5;  //parameter for rank based selection
	static double mutationVariation = 0.5; //parameter for mutation variation
	...
	//replace population with other population (mu + lambda) selection
		...
		//replace population with other population (mu, lambda) selection
		public void fitness_replacement(double[] fitness_childs, double[][] pop_childs, double[] pop_fitness, double[][] population) {
			orderByFitness(fitness_childs, pop_childs);
	...
	public void run()
	{
        ...
        while(evals<evaluations_limit_){
            ...
            fitness_replacement(children_fitness, children, pop_fitness, population);
            evals++;;
        }
